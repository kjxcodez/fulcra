 ...
 * Fulcra Card
 *
 * Elevation rule (DESIGN.md §4.4):
 *   Cards in normal document flow → border-only (elevation-0)
 *   No box-shadow on in-flow cards.
 *
 * Radius: rounded-md (6px — radius-md)
 * Surface: bg-card (#FFFFFF raised)
 * Border: border-border (#DBDCD5 hairline)
 */
function Card({
  className,
  size = "default",
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "py-(--card-spacing) text-sm text-card-foreground",
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-md border-t border-border bg-muted/40 p-(--card-spacing)",
        className
      )}
      {...props}
    />
